...
class nn:
	...
	def __init__(self, x1, y_, y_string, shapes_batch, softmax_num,is_training,input_dim, is_batchnorm):
		...
		with tf.compat.v1.variable_scope("softmax") as scope:
			...
			# Create loss
			self.loss    = tf.reduce_mean(input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y_, logits=self.o1),name="loss_mean")
		self.label=y_
	...
	def net(self,x, shapes_batch,softmax_num,is_training, input_dim, is_batchnorm):   
		...
		mean = tf.reduce_sum(input_tensor=conv4r,axis=1,keepdims=True)/shape_list
		var = tf.sqrt( tf.reduce_sum( input_tensor=tf.square(conv4r-mean),axis=1,keepdims=True)/shape_list )
	...
	def fc_layer(self, bottom, n_weight, name):
		...
		b = tf.compat.v1.get_variable(name+'b', dtype=tf.float32, initializer=tf.random.uniform([n_weight],-0.001,0.001, dtype=tf.float32))
	...
	def conv_layer(self, bottom, kernel_size,num_channels, stride, depth, name, shape_list):   # n_prev_weight = int(bottom.get_shape()[1])
		n_prev_weight = tf.shape(input=bottom)[1]

		inputlayer=bottom
		initer = tf.compat.v1.truncated_normal_initializer(stddev=0.1)

		W = tf.compat.v1.get_variable(name+'W', dtype=tf.float32, shape=[kernel_size, num_channels, depth], initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
		b = tf.compat.v1.get_variable(name+'b', dtype=tf.float32, initializer=tf.constant(0.001, shape=[depth], dtype=tf.float32))
		
		conv =  ( tf.nn.bias_add( tf.nn.conv1d(input=inputlayer, filters=W, stride=stride, padding='SAME'), b))
		
		mask = tf.sequence_mask(shape_list,tf.shape(input=conv)[1]) # make mask with batch x frame size
		mask = tf.compat.v1.where(mask, tf.ones_like(mask,dtype=tf.float32), tf.zeros_like(mask,dtype=tf.float32))
		mask=tf.tile(mask, tf.stack([tf.shape(input=conv)[2],1])) #replicate make with depth size
		mask=tf.reshape(mask,[tf.shape(input=conv)[2], tf.shape(input=conv)[0], -1])
		mask = tf.transpose(a=mask,perm=[1, 2, 0])
		print (mask)
	...
	def batch_norm_wrapper_1dcnn(self, inputs, is_training, name, shape_list, is_batchnorm,decay = 0.999 ):
		if is_batchnorm:
			shape_list = tf.cast(shape_list, tf.float32)
			epsilon = 1e-3
			scale = tf.compat.v1.get_variable(name+'scale',dtype=tf.float32,initializer=tf.ones([inputs.get_shape()[-1]]) )
			beta = tf.compat.v1.get_variable(name+'beta',dtype=tf.float32,initializer= tf.zeros([inputs.get_shape()[-1]]) )
			pop_mean = tf.compat.v1.get_variable(name+'pop_mean',dtype=tf.float32,initializer = tf.zeros([inputs.get_shape()[-1]]), trainable=False)
			pop_var = tf.compat.v1.get_variable(name+'pop_var',dtype=tf.float32,initializer = tf.ones([inputs.get_shape()[-1]]), trainable=False)
			if is_training:
			#batch_mean, batch_var = tf.nn.moments(inputs,[0,1])
				batch_mean = tf.reduce_sum(input_tensor=inputs,axis=[0,1])/tf.reduce_sum(input_tensor=shape_list) # for variable length input
				batch_var = tf.reduce_sum(input_tensor=tf.square(inputs-batch_mean), axis=[0,1])/tf.reduce_sum(input_tensor=shape_list) # for variable length input
	...
	def batch_norm_wrapper_fc(self, inputs, is_training, name, is_batchnorm, decay = 0.999 ):
		if is_batchnorm:
			...
			pop_var = tf.compat.v1.get_variable(name+'pop_var',dtype=tf.float32,initializer = tf.ones([inputs.get_shape()[-1]]), trainable=False)
			if is_training:
				batch_mean, batch_var = tf.nn.moments(x=inputs,axes=[0])
				train_mean = tf.compat.v1.assign(pop_mean, pop_mean * decay + batch_mean * (1 - decay))
				train_var = tf.compat.v1.assign(pop_var, pop_var * decay + batch_var * (1 - decay))
				with tf.control_dependencies([train_mean, train_var]):
					return tf.nn.batch_normalization(inputs,batch_mean, batch_var, beta, scale, epsilon)
